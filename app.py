...
def extract_genre_names(genres_str):
    """Extract genre names from the genres string"""
    try:
...
def search_by_title(title):
    """Improved search that finds the best matching movie"""
    ...
    # If no exact matches, use TF-IDF similarity
...
def parse_genres(genres_str):
    """Parse genres from string format to list for display"""
    try:
...
def api_search():
    try:
        ...
        for _, row in results.iterrows():
            # Safely get poster_path with default value
